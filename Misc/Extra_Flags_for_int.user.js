...
var setup = {
    ...
    html: function () {
        ...
        var filters = '<form action="">' +
            '<input type="radio" name="' + shortId + 'filter" value="showAll" checked>Show all regional flags<br>' +
            '<input type="radio" name="' + shortId + 'filter" value="firstOnly">Show only the country and first regional flag<br>' +
            '<input type="radio" name="' + shortId + 'filter" value="lastOnly">Show only the country and last regional flag (v1 regional flags style)</form>';

        if (regions.length > 1) {
            return htmlFixedStart + '<div>Region: <br/><select id="' + shortId + 'countrySelect">' +
                '</select></div><br/>' + htmlBackNextButtons +
                '<br/>' + htmlSaveButton + '</div>' + htmlHelpText + filters;
        }
        ...
        return htmlFixedStart + '<div>Country: <br/><select id="' + shortId + 'countrySelect">' +
            '</select></div><br/>' + htmlBackNextButtons + '<br/>' + htmlHelpText + filters;
    ...
    show: function () {
        ...
        if (filter !== "none") {
            ...
            document.querySelector('input[value=showAll]').checked = true;
        }
        ...
        setup.q('save').addEventListener('click', function () {
            ...
            filter = "";
            var filterSelected = document.querySelector('input[name="' + shortId + 'filter"]:checked').value;
    ...
    jsonData.forEach(function (post) {
        ...
        if (postedRegions.length > 0) {
            ...
            for (var i = 0; i < postedRegions.length; i++) {
                ...
                if ((filter === "firstOnly" && i === 0) || (filter === "lastOnly" && i === postedRegions.length - 1) || filter === "showAll") {
